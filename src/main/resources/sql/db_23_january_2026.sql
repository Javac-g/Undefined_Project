...
INSERT INTO roles(name,description) VALUES ('USER','customer role');
...
CREATE TABLE purchase_fees (
    ...
    max_amount NUMERIC(12,2),         -- NULL = no upper limit
...
INSERT INTO purchase_fees (
    plan_id, min_amount, max_amount, fee_amount
)
VALUES
    (1, 0.00, 100.00, 5.00),
    (1, 100.00, NULL, 15.00);
...
VALUES (
    'BASIC',
    'For individuals who want to try out Pines Digital.
Up to 1 person (1 Owner / 1 Admin).
...
- Client self-management via control panel
- Writing version and difference management
- Email notifications for contacts and deadlines
- Order receipts management panel

Price: $0/month (tax included).',
    INTERVAL '1 month',
    0.00,
    'USD'
);
...
VALUES (
    'PREMIUM',
    'For individuals who manage a lot of projects.
Up to 1 person (1 Owner / 2 Admins - Free)

Includes all Basic features, plus:
- Hosting configuration assistance
- SSL certificate setup support
- Private Email (PE) mailbox setup
- Premium DNS included
- DNS routing and SSL configuration support
- Customer verification support & assistance
- Access to premium domains
- Email support (response within 24 hours)
- No fee for purchases under $100
- 10% fee for purchases over $100

Price: $50/month (tax included).',
    INTERVAL '1 month',
    50.00,
    'USD'
);
INSERT INTO purchase_fees (
    plan_id, min_amount, max_amount, fee_amount, currency
)
VALUES
    (2, 0.00, 100.00, 0.00, 'USD'),        -- transactions under $100
    (2, 100.00, NULL, 10.00, 'USD');       -- transactions $100 and above (10% of amount handled in app)
...
VALUES (
    'BUSINESS',
    'For corporate and business teams that manage domain names and IT services as a team.
Up to 99 people, pay per member.

Includes all Premium features, plus:
- Free general TLD domain (.com/.net/.org) for 1st year
- Premium domain search & handling
- Business account setup & verification
- Registrar flexibility (Namecheap, GoDaddy, etc.)
- KYC & identity verification support
- Support via email (2h response)
- Only 10% fee for premium domain purchases
- Priority support

Price: $250 per person per month.',
    INTERVAL '1 month',
    250.00,
    'USD'
);
...
VALUES
    (3, 0.00, NULL, 10.00, 'USD');  -- 10% fee for premium domain purchases
...
VALUES (
    'ENTERPRISE',
    'Tailored for large businesses and mission-critical platforms: managed infrastructure, compliance support, custom integrations, and dedicated engineering.

Unlimited users. Annual or invoice payment. Bank transfer supported.

Includes all Business features, plus:
- Dedicated domain manager: renewals, DNS, support
- Full domain lifecycle: acquire → renew → resell
- Sanctioned region help: crypto, legal routing
- Full hands-off service management
- VIP support: 1h response SLA
- Only 5% fee for premium domain purchases
...
SELECT * FROM membership_plans;
ALTER TABLE membership_plans
ALTER COLUMN duration TYPE INTEGER USING EXTRACT(DAY FROM duration);
